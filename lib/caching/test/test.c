...
main(void)
{
	...
	memc = sstack_cache_init(config, 1);
	if (NULL == memc) {
	...
	ret = sstack_cache_store(memc, key, value, strlen(value)+1);	
	if (ret != 0) {
	...
	// Read back the value
	ptr = sstack_cache_read_one(memc, key, strlen(key), (size_t *)&len);
	if (NULL == ptr) {
